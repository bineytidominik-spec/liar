    ...
    <div className="fade-up">
      ...
      <div className="flex gap-2 mb-6">
        <input
          ...
          maxLength={20}
          className="flex-1 bg-white border border-stone-200 px-4 py-3 focus:outline-none focus:border-rose-400 transition-colors text-stone-800 placeholder:text-stone-400 rounded-xl"
        />
        <button
          onClick={handleAdd}
          disabled={!name.trim() || players.length >= 15}
          className="px-5 bg-rose-500 text-white font-mono-game text-xs uppercase tracking-wider hover:bg-rose-600 disabled:opacity-30 disabled:hover:bg-rose-500 transition-colors rounded-xl"
        >
          + Hinzu
        </button>
      </div>
      ...
      <div className="space-y-1.5 mb-8 max-h-[50vh] overflow-y-auto">
        ...
        {players.map((p, i) => (
          <div key={p} className="group flex items-center justify-between bg-white border-l-4 border-rose-400 px-4 py-2.5 fade-up rounded-xl shadow-sm" style={{ animationDelay: `${i * 30}ms` }}>
      ...
      <button
        ...
        className="w-full py-4 bg-rose-500 hover:bg-rose-600 text-white font-mono-game text-sm uppercase tracking-[0.2em] disabled:bg-stone-200 disabled:text-stone-400 transition-colors rounded-xl shadow-sm"
      >
        Weiter zur Runde →
      </button>
    </div>
